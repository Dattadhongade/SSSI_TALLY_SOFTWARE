...
export default function GroupCreate() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/account-groups', formData);
      Swal.fire({
        icon: 'success',
        title: 'Saved',
      ...
      setFormData({ name: '', alias: '', parentGroupId: '', isRevenue: false });
      fetchGroups(); // refresh parent dropdown
    } catch (err) {
      console.error(err);
    ...
    <div className="bg-tally-bg text-tally-dark min-h-full font-sans p-4">
      <div className="max-w-2xl mx-auto bg-white border border-tally-border shadow-sm">
        <div className="bg-[#f0f6fa] border-b border-tally-border px-4 py-2 font-bold text-tally-blue flex justify-between">
          ...
          <span className="text-xs text-gray-500">Account Master</span>
        </div>
        ...
        <form ref={formRef} onKeyDown={handleEnterToNextField} onSubmit={handleSubmit} className="p-6">
          ...
          <div className="flex items-center mb-4">
            <label className="w-48 text-sm font-bold text-tally-blue">Under <span className="text-red-500">*</span></label>
            <span className="mx-2 text-tally-blue font-bold">:</span>
            <select
              required
              name="parentGroupId"
          ...
          <div className="flex items-center mb-4">
            <label className="w-48 text-sm text-tally-blue font-bold">Nature of Group</label>
            <span className="mx-2 text-tally-blue font-bold">:</span>
            <select
              name="isRevenue"
              value={formData.isRevenue}
              onChange={(e) => setFormData({...formData, isRevenue: e.target.value === 'true'})}
              className="flex-1 border border-tally-border px-1 focus:bg-tally-yellow outline-none font-bold"
            >
              <option value="false">Assets / Liabilities (Balance Sheet)</option>
              <option value="true">Income / Expenses (Profit & Loss A/c)</option>
            </select>
          </div>
