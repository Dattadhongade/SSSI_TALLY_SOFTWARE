...
export default function ReceiptVoucher() {
  ...
  const [formData, setFormData] = useState({
    ...
    voucherNumber: '',
    accountId: '', // Cash or Bank (Debit)
    partyId: '', // Customer / Income (Credit)
    amount: '',
    narration: '',
  });
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const type = vTypesRes.data.find(v => v.name === 'Receipt');
        if (type) {
  ...
  const handleSubmit = async (e) => {
    ...
    const entries = [
      { ledgerId: formData.accountId, debitAmount: amt, creditAmount: 0 }, // Dr Cash/Bank
      { ledgerId: formData.partyId, debitAmount: 0, creditAmount: amt }    // Cr Party
    ];
    ...
    try {
      ...
      Swal.fire({ icon: 'success', title: 'Saved', text: `Receipt Voucher ${res.data.voucherNumber} created!`, timer: 1500, showConfirmButton: false });
  ...
  // In a real app, filter ledgers by groups. Here we show all for simplicity.
      ...
      <form onSubmit={handleSubmit} onKeyDown={handleEnterToNextField} className="bg-white border border-tally-border shadow-sm flex flex-col h-[85vh]">
        ...
        <div className="bg-[#f0f6fa] border-b border-tally-border p-2 flex justify-between items-start">
          <div className="flex gap-4">
            <div className="font-bold text-lg text-tally-blue">Receipt</div>
        ...
        <div className="p-4 space-y-4 flex-1">
          ...
          <div className="flex items-center">
            ...
            <select name="partyId" value={formData.partyId} onChange={handleFormChange} className="w-64 border border-tally-border px-1 py-0.5 focus:bg-tally-yellow focus:outline-none" required>
              <option value="">Select Party/Income...</option>
              {otherLedgers.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
            </select>
          </div>
