...
export default function GSTR2Report() {
  ...
  useEffect(() => {
    setPageTitle('GSTR-2 Report');
    const fetchReport = async () => {
      try {
        setLoading(true);
        const res = await api.get('/gst/gstr2', { params: dateRange });
        setReportData(res.data);
        setError(null);
      } catch (err) {
        console.error(err);
        setError(err.response?.data?.message || 'Failed to fetch GSTR-2');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="p-4 text-tally-blue font-bold">Loading GSTR-2...</div>;
  }
  ...
  const handleExportJson = () => {
    ...
    a.href = url;
    a.download = `GSTR2_${dateRange.startDate}_to_${dateRange.endDate}.json`;
    a.click();
  ...
  return (
    <div className="bg-tally-bg text-tally-dark min-h-full font-sans p-4">
      <div className="bg-white border border-tally-border shadow-sm p-4 mb-4 flex justify-between items-center">
        <div>
          <h2 className="text-xl font-bold text-tally-blue">GSTR-2 (Inward Supplies)</h2>
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        ...
        <div className="bg-white border border-tally-border p-4 shadow-sm text-center">
          <div className="text-xs font-bold text-gray-500 uppercase">Total ITC (Tax Amount)</div>
      ...
      <div className="bg-white border border-tally-border shadow-sm mb-4">
        <div className="bg-[#f0f6fa] border-b border-tally-border px-4 py-2 font-bold text-tally-blue">
          B2B Invoices (Purchases from Registered Dealers)
        </div>
      ...
      <div className="bg-white border border-tally-border shadow-sm">
        <div className="bg-[#f0f6fa] border-b border-tally-border px-4 py-2 font-bold text-tally-blue">
          B2BUR (Purchases from Unregistered Dealers)
        </div>
        <div className="p-4 overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            <thead>
            ...
            <tbody>
              {reportData.b2bur.length === 0 ? (
                <tr><td colSpan="6" className="p-4 text-center text-gray-500 italic">No Unregistered Purchases found</td></tr>
              ) : (
                reportData.b2bur.map(inv => {
                  const totalTax = Number(inv.cgst) + Number(inv.sgst) + Number(inv.igst) + Number(inv.cess);
                  return (
                    <tr key={inv.id} className="border-b border-gray-200 hover:bg-[#fcf8e3]">
                      <td className="p-2 font-bold">{inv.voucherNumber}</td>
